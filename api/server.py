# api/server.py (Versão Final e Completa para Gumroad)

import os
import requests
from flask import Flask, request, jsonify

app = Flask(__name__)

# --- Configuração ---
# O seu token de acesso secreto do Gumroad, lido das variáveis de ambiente do Vercel.
GUMROAD_ACCESS_TOKEN = os.environ.get('GUMROAD_ACCESS_TOKEN')
GUMROAD_API_URL = "https://api.gumroad.com/v2/licenses/verify"

@app.route('/', methods=['POST'])
def validate_license():
    # 1. Verifica se o servidor está configurado com o token secreto.
    if not GUMROAD_ACCESS_TOKEN:
        return jsonify({"success": False, "message": "Erro de configuração do servidor."}), 500

    # 2. Recebe os dados do aplicativo desktop.
    license_key = request.form.get('license_key')
    product_permalink = request.form.get('product_permalink')

    if not license_key or not product_permalink:
        return jsonify({"success": False, "message": "Dados da licença ou do produto não fornecidos."}), 400
        
    # 3. Prepara a chamada para a API oficial do Gumroad.
    payload = {
        'access_token': GUMROAD_ACCESS_TOKEN,
        'product_permalink': product_permalink,
        'license_key': license_key.strip()
    }
    
    try:
        # 4. Envia a requisição para o Gumroad.
        response = requests.post(GUMROAD_API_URL, data=payload, timeout=15)
        response.raise_for_status()
        
        data = response.json()
        
        # 5. Processa a resposta e a retorna de forma padronizada.
        if data.get("success"):
            uses = data.get("purchase", {}).get("uses", 0)
            MAX_ACTIVATIONS = 3 # Defina seu limite de ativações aqui!
            
            if uses <= MAX_ACTIVATIONS:
                return jsonify({
                    "valid": True,
                    "message": f"Licença válida. Ativação {uses} de {MAX_ACTIVATIONS}.",
                    "meta": {"status": "active"}
                })
            else:
                return jsonify({
                    "valid": False,
                    "error": "Esta chave de licença atingiu o número máximo de ativações."
                })
        else:
            return jsonify({
                "valid": False,
                "error": data.get("message", "Chave de licença inválida.")
            })

    except requests.RequestException as e:
        return jsonify({"success": False, "message": f"Não foi possível conectar ao ser
